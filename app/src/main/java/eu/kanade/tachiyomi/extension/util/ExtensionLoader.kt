...
internal object ExtensionLoader {
    ...
    private val SUPPORTED_LIB_VERSIONS = setOf(1.4, 1.6)

    fun isSupportedLibVersion(version: Double?): Boolean = version != null && version in SUPPORTED_LIB_VERSIONS
    ...
    private suspend fun loadExtension(context: Context, extensionInfo: ExtensionInfo): LoadResult {
        ...
        if (!isSupportedLibVersion(libVersion)) {
            logcat(LogPriority.WARN) {
                "Lib version is $libVersion, while only version(s) " +
                    SUPPORTED_LIB_VERSIONS.sorted().joinToString() + " are supported"
            }
            return LoadResult.Error
        }
        val supportedLibVersion = checkNotNull(libVersion)
        ...
        val extension = Extension.Installed(
            ...
            libVersion = supportedLibVersion,
            lang = lang,
            isNsfw = isNsfw,
            sources = sources,
