...
object Waifu2x {
    ...
    private var isRealCuganInitialized = false
    private var isRealEsrganInitialized = false
    private var isNoseInitialized = false
    private var isWaifu2xInitialized = false
    private var isAnime4kInitialized = false
    ...
    fun init(context: Context, noiseLevel: Int = 2, scale: Int = 2): Boolean = synchronized(this) {
    ...
    private var lastRealCuganConfig: RealCuganConfig? = null

    fun initRealCugan(context: Context, noiseLevel: Int, scale: Int, isPro: Boolean = false, tileSleepMs: Int = 0, tileSize: Int = 128): Boolean = synchronized(this) {
        val newConfig = RealCuganConfig(noiseLevel, scale, isPro)
        
        // Force reinit only if model parameters changed (not tileSleepMs)
        if (lastRealCuganConfig != newConfig) {
            android.util.Log.d("Waifu2x", "Config changed from $lastRealCuganConfig to $newConfig, reinitializing...")
